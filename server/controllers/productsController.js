...
export const getFilterProducts = async (req, res) => {
	...
	try {
		...
		const filterProducts = products.filter(product => {
			const nameCondition = !params.name || product.name.includes(params.name)

			const idCondition = !params.product_id || product.product_id.includes(params.product_id)

			const categoryCondition = !params.category || product.category === params.category
...
export const createProduct = async (req, res) => {
	const { name, description, quantity, category, details, product_id, EAN } = req.body
	...
	try {
		...
		if (existingProduct) {
			return res.status(400).json({ message: 'You already have product with this ID.' })
		}
...
export const updateProduct = async (req, res) => {
	const { product_id, name, quantity, category, image, EAN, details } = req.body

	const updatedData = { product_id, name, quantity, category, EAN: '', details: JSON.parse(details) }
