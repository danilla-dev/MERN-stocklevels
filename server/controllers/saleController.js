...
export const getSales = async (req, res) => {
	...
	try {
		const salesByDate = await Sale.find({ user_id, createdAt: { $gte: startDate, $lte: endDate } }).sort({
			createdAt: -1,
		})
...
export const getSalesOfProduct = async (req, res) => {
	...
	try {
		const sale = await Sale.find({ user_id, product_id }).sort({ createdAt: -1 })
...
export const postSale = async (req, res) => {
	...
	try {
		...
		await Product.updateOne(
			{ user_id, product_id },
			{ $set: { quantity: productQuantity - quantity, sales: [...productSalesArray, saleID] } }
		)
