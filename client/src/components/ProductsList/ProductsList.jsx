...
const ProductsList = () => {
	...
	const findProduct = (EAN, e) => {
		const scannedProduct = products.filter(product => product.EAN === Number(EAN))[0]
		if (scannedProduct) {
			navigate(`product/details/${scannedProduct.product_id}`)
			togglePopup(e, 'lg', <Product />)
		} else {
			navigate('product/add')
			togglePopup(e, 'sm', <NewProductForm ean={EAN} />)
		}
		bufferRef.current = ''
	}
